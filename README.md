...
- [5 - Helm](#5---helm)
- [6 - Istio as a service Mesh](#6---istio-as-a-service-mesh)
  - [A - Istiod](#a---istiod)
  - [B - Envoy](#b---envoy)
- [7 - Accessing the cluster](#7---accessing-the-cluster)
  - [Istio Gateway](#istio-gateway)
- [8 - Distributed tracing](#8---distributed-tracing)
  - [A - Meet Jaeger](#a---meet-jaeger)
  - [B - Context propagation](#b---context-propagation)
  - [C - So what do traces look like in Jaeger UI?](#c---so-what-do-traces-look-like-in-jaeger-ui)
- [9 - Grafana dashboard](#9---grafana-dashboard)
  - [A - Loki stack](#a---loki-stack)
  - [B - Configure Loki Derived Field to extract Trace ID from the logs](#b---configure-loki-derived-field-to-extract-trace-id-from-the-logs)
  - [C - Configure trace to logs](#c---configure-trace-to-logs)
  - [D - Metrics](#d---metrics)
    - [Introducing exemplars](#introducing-exemplars)
- [10 - Kiali](#10---kiali)
...
Since this demo repo should be self-contained we put cluster configuration, app code and CICD definition in the same place but in practice `business-logic`, `front-end` and `helm` should have their own repositories.
...
Note that the default sampling rate is 1%, which means that 99% of the traces won't get reported. For testing purpose we want to see all traces so I've set the sampling rate to 100% in [tracing.yaml](./istio-add-ons/tracing.yaml)
...
Grafana's data sources, namely Jaeger and Loki are defined in [grafana.yaml](./istio-add-ons/grafana.yaml#L44) and there are some tweaks to be made to correlate logs, traces and metrics.

### B - Configure Loki Derived Field to extract Trace ID from the logs

The [Derived Fields configuration](./istio-add-ons/grafana.yaml#L70) helps you add fields parsed from the log message and add a link that uses the value of the field. In our case we want to extract the Trace ID from the log message. This is necessary if you want to get this "Jaeger" button showing up in your logs.

<img src="./docs/img/logs_to_trace.png" width="850"/>

### C - Configure trace to logs

For the traces to be correlated to logs we need to match the [service.name](./business-logic/pages/views.py#L24) trace tag with the pod tag `app`, this is done in the [Jaeger data source definiton](./istio-add-ons/grafana.yaml#L97)
...
I have reused the [Django Prometheus dashboard available on Grafana Labs](https://grafana.com/grafana/dashboards/9528-django-prometheus/) as it is quite descriptive and provides the information we need to analyze the traffic coming into our back-end application. The dashboard is configured in [grafana.yaml](./istio-add-ons/grafana.yaml) where it is stored in a k8s config map which is itself mounted as a volume in the Grafana pod.
...
#### Implement exemplar in django-prometheus (WIP)

[cf open issue I have raised on Github](https://github.com/korfuri/django-prometheus/issues/345)

## 10 - Kiali

Kiali is a dashboard for Istio service mesh where you can visualize traffic in real-time. An example graph of the kind-e2e app is shown below:

<img src="./docs/img/kiali.png" width="650"/>

The dashboard is made available at [http:localhost:8080/kiali](http:localhost:8080/kiali) through the Istio gateway.
...
- Finish implementing exemplars in django-prometheus metrics ([cf open issue I have raised](https://github.com/korfuri/django-prometheus/issues/345))
- Use [helm-file](https://github.com/helmfile/helmfile) to manage chart releases across several namespaces
- Add logs in front-end and correlate them with trace ID
- Add helm [auto-documentation](https://github.com/norwoodj/helm-docs)
